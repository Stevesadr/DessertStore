...
import ProductShopingCard from "./Btns/ProductShopingCard";
import { zeroPrice } from "../../func/CalculationZeroCount";
import FinishingTheOrder from "./FinishingTheOrder";
...
const ShopingCard = () => {
  ...
  return (
    <div className="w-full m-4 lg:w-2/3 rounded-lg h-full p-4 bg-constom-50 ">
      <h2 className="text-constom-red font-RedHatText-Bold text-[24px]">
        Your Card ({data.allCount})
      </h2>
      {data.allCount === 0 ? (
        <div className="flex flex-col items-center justify-center m-4">
          ...
          <p className="text-stone-600 text-[13px] font-bold">
            Your added items will appear hear
          </p>
        </div>
      ) : (
        <div className="font-RedHatText-Bold text-[30px] p-4 ">
          {data.items.map((item) => {
            return <ProductShopingCard product={item} />;
          })}
          ...
          <div className="w-full bg-constom-100 p-4 my-2 rounded-xl flex items-center justify-center">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="21"
              height="20"
            ...
            <h5 className="text-[11px] font-RedHatText-Regular">
              This is a{" "}
              <span className="font-RedHatText-Bold"> carbon-neutral </span>{" "}
              delivery
            </h5>
          </div>

          <button
            className="w-full text-[13px] text-constom-50 bg-constom-red p-3 lg:p-4 mt-4 rounded-full flex items-center justify-center"
            onClick={actionforopne}
          >
            Confirm Order
          </button>
          {open === false ? null : <FinishingTheOrder action={actionforopne} />}
        </div>
      )}
    </div>
  );
